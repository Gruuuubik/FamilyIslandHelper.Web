	...
	public class HomeControllerTest : BaseTest
	{
		...
		[Description(@"
			...
			4. Select Item 2 from current building.
				...
				4.2. Check that components tree contains all necessary components.
			5. Select producable item in components tree.
				5.1. Check that producable item is not visible.
			6. Turn on showing of components with time and energy
				6.1. Check that item info is shown.
			7. Set ItemCount = 2.
		...
		public void HomePageTest(string testDataJsonFileName)
		{
			try
			{
				...
				//6
				driver.FindElement(By.Id("ShowComponentsWithTimeAndEnergy")).Click();
